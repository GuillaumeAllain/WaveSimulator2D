...
class WaveSimulator2D:
    ...
    def __init__(self, w, h):
        ...
        self.source_opacity = (
            0.9  # opacity of source pixels to incoming waves. If the opacity is 0.0
        )
        ...
        self.d = np.ones((h, w), dtype=np.float32)  # dampening field
        self.u = np.zeros((h, w), dtype=np.float32)  # field values
        self.u_prev = np.zeros((h, w), dtype=np.float32)  # field values of prev frame
        ...
        # self.laplacian_kernel = np.array([[0.103, 0.147, 0.103],
        #                                   [0.147, -1.0, 0.147],
        #                                   [0.103, 0.147, 0.103]])
    ...
    def update_field(self):
        ...
        # )
        # laplacian = laplace(self.u) / 4

        # update field
        # v = (self.u - self.u_prev) * self.d
        # r = self.u + v + laplacian * (self.c * self.dt) ** 2

        self.u_prev[:], self.u[:] = self.u, (
            self.u
            + ((self.u - self.u_prev) * self.d)
            + ((laplace(self.u) * (self.cdt) ** 2) / 4)
        )
    ...
    def update_sources(self):
        ...
        """
        v = (
            np.sin(self.sources[:, 2] + self.sources[:, 4] * self.t)
            * self.sources[:, 3]
        )
        coords = self.sources[:, 0:2].astype(np.int32)

        t = self.source_opacity
        self.u[coords[:, 1], coords[:, 0]] = self.u[
            coords[:, 1], coords[:, 0]
        ] * t + v * (1.0 - t)
